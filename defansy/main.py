...
from defansy.config import config, configurable_keys, configurable_boolean_keys
from defansy.io import slugify, e, fformat, cprint, debug, info, warn, error, fail, CommandStreamReader
from defansy.plugins import Pattern, PortScan, ServiceScan, Report, defansy
from defansy.targets import Target, Service
...
if not os.path.exists(config['config_dir']):
	...
	if not os.path.exists(os.path.join(config['config_dir'], 'VERSION-' + VERSION)):
		warn('It looks like the config in ' + config['config_dir'] + ' is outdated. Please remove the ' + config['config_dir'] + ' directory and re-run defansy to rebuild it.')
...
if not os.path.exists(config['data_dir']):
	...
	if not os.path.exists(os.path.join(config['data_dir'], 'VERSION-' + VERSION)):
		warn('It looks like the plugins in ' + config['data_dir'] + ' are outdated. Please remove the ' + config['data_dir'] + ' directory and re-run defansy to rebuild them.')
...
def cancel_all_tasks(sig, frame):
	...
	for target in defansy.scanning_targets:
...
async def get_semaphore(defansy):
	semaphore = defansy.service_scan_semaphore
	while True:
		# If service scan semaphore is locked, see if we can use port scan semaphore.
		if semaphore.locked():
			if semaphore != defansy.port_scan_semaphore: # This will be true unless user sets max_scans == max_port_scans
				...
				if not defansy.pending_targets and (config['max_port_scans'] - port_scan_task_count) >= 1: # If no more targets, and we have room, use port scan semaphore.
					if defansy.port_scan_semaphore.locked():
						await asyncio.sleep(1)
						continue
					semaphore = defansy.port_scan_semaphore
					break
				else: # Do some math to see if we can use the port scan semaphore.
					if (config['max_port_scans'] - (port_scan_task_count + (len(defansy.pending_targets) * config['port_scan_plugin_count']))) >= 1:
						if defansy.port_scan_semaphore.locked():
							await asyncio.sleep(1)
							continue
						semaphore = defansy.port_scan_semaphore
...
async def port_scan(plugin, target):
	...
	async with target.defansy.port_scan_semaphore:
...
async def service_scan(plugin, service):
	semaphore = service.target.defansy.service_scan_semaphore

	if not config['force_services']:
		semaphore = await get_semaphore(service.target.defansy)
	...
	while plugin_pending:
		...
		if plugin.max_global_instances and plugin.max_global_instances > 0:
			async with service.target.defansy.lock:
				# Count currently running plugin instances.
				for target in service.target.defansy.scanning_targets:
		...
		async with semaphore:
			...
			nmap_extra = service.target.defansy.args.nmap
			if service.target.defansy.args.nmap_append:
				nmap_extra += ' ' + service.target.defansy.args.nmap_append
...
async def generate_report(plugin, targets):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(defansy)
...
async def scan_target(target):
	...
	if config['force_services']:
		...
		for plugin in target.defansy.plugin_types['port']:
			...
			if config['port_scans'] and plugin.slug in config['port_scans']:
				...
				matching_tags = False
				for tag_group in target.defansy.tags:
					if set(tag_group).issubset(plugin_tag_set):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in target.defansy.excluded_tags:
					if set(tag_group).issubset(plugin_tag_set):
						excluded_tags = True
						break
	...
	async with defansy.lock:
		defansy.scanning_targets.append(target)
	...
	while pending:
		...
		for service in services:
			...
			nmap_extra = target.defansy.args.nmap
			if target.defansy.args.nmap_append:
				nmap_extra += ' ' + target.defansy.args.nmap_append
			...
			for plugin in target.defansy.plugin_types['service']:
			...
			if not service_match:
				...
				if service.name not in config['service_exceptions'] and service.full_tag() not in target.defansy.missing_services:
					target.defansy.missing_services.append(service.full_tag())

	for plugin in target.defansy.plugin_types['report']:
	...
	async with defansy.lock:
		defansy.completed_targets.append(target)
		defansy.scanning_targets.remove(target)
...
async def run():
	...
	parser.add_argument('-c', '--config', action='store', type=str, default=config_file, dest='config_file', help='Location of defansy\'s config file. Default: %(default)s')
	parser.add_argument('-g', '--global-file', action='store', type=str, help='Location of defansy\'s global file. Default: %(default)s')
	...
	parser.add_argument('--timeout', action='store', type=int, help='Specifies the maximum amount of time in minutes that defansy should run for. Default: %(default)s')
	...
	parser.add_argument('--proxychains', action='store_true', help='Use if you are running defansy via proxychains. Default: %(default)s')
	...
	parser.add_argument('--accessible', action='store_true', help='Attempts to make defansy output more accessible to screenreaders. Default: %(default)s')
	parser.add_argument('-v', '--verbose', action='count', help='Enable verbose output. Repeat for more verbosity.')
	parser.add_argument('--version', action='store_true', help='Prints the defansy version and exits.')
	parser.error = lambda s: fail(s[0].upper() + s[1:])
	args, unknown = parser.parse_known_args()

	errors = False

	defansy.argparse = parser

	if args.version:
		print('defansy v' + VERSION)
		sys.exit(0)
	...
	if not args.config_file:
		unknown_help()
		fail('Error: Could not find config.toml in the current directory or ~/.config/defansy.')
	...
	if not config['plugins_dir']:
		unknown_help()
		fail('Error: Could not find plugins directory in the current directory or ~/.config/defansy.')
	...
	for plugins_dir in plugins_dirs:
		for plugin_file in sorted(os.listdir(plugins_dir)):
			if not plugin_file.startswith('_') and plugin_file.endswith('.py'):
				...
				try:
					spec = importlib.util.spec_from_file_location("defansy." + filename[:-3], os.path.join(dirname, filename))
					plugin = importlib.util.module_from_spec(spec)
					spec.loader.exec_module(plugin)

					clsmembers = inspect.getmembers(plugin, predicate=inspect.isclass)
					for (_, c) in clsmembers:
						if c.__module__ in ['defansy.plugins', 'defansy.targets']:
							continue
	...
	for plugin in defansy.plugins.values():
		if plugin.slug in defansy.taglist:
			unknown_help()
	...
	if len(defansy.plugin_types['port']) == 0:
		unknown_help()
		fail('Error: There are no valid PortScan plugins in the plugins directory "' + config['plugins_dir'] + '".')

	# Sort plugins by priority.
	defansy.plugin_types['port'].sort(key=lambda x: x.priority)
	defansy.plugin_types['service'].sort(key=lambda x: x.priority)
	defansy.plugin_types['report'].sort(key=lambda x: x.priority)

	if not config['global_file']:
		unknown_help()
		fail('Error: Could not find global.toml in the current directory or ~/.config/defansy.')
						...
						if 'pattern' in pattern:
							try:
								...
								if 'description' in pattern:
									defansy.patterns.append(Pattern(compiled, description=pattern['description']))
								else:
									defansy.patterns.append(Pattern(compiled))
							except re.error:
								unknown_help()
		...
		if key == 'global' and isinstance(val, dict): # Process global plugin options.
			for gkey, gval in config_toml['global'].items():
				if isinstance(gval, bool):
					for action in defansy.argparse._actions:
						if action.dest == 'global.' + slugify(gkey).replace('-', '_'):
							if action.const is True:
								action.__setattr__('default', gval)
							break
				else:
					if defansy.argparse.get_default('global.' + slugify(gkey).replace('-', '_')):
						defansy.argparse.set_defaults(**{'global.' + slugify(gkey).replace('-', '_'): gval})
		elif isinstance(val, dict): # Process potential plugin arguments.
			for pkey, pval in config_toml[key].items():
				if defansy.argparse.get_default(slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_')) is not None:
					for action in defansy.argparse._actions:
						if action.dest == slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'):
							if action.const and pval != action.const:
								...
								errors = True
							break
					defansy.argparse.set_defaults(**{slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'): pval})
		else: # Process potential other options.
			key = key.replace('-', '_')
			if key in configurable_keys:
				other_options.append(key)
				config[key] = val
				defansy.argparse.set_defaults(**{key: val})
	...
	if args.list:
		...
		if type in ['plugin', 'plugins', 'port', 'ports', 'portscan', 'portscans']:
			for p in defansy.plugin_types['port']:
	...
	if config['max_plugin_target_instances']:
		for plugin_instance in config['max_plugin_target_instances']:
			...
			if len(plugin_instance) == 2:
				if plugin_instance[0] not in defansy.plugins:
	...
	for slug, plugin in defansy.plugins.items():
	...
	if not errors:
		if config['force_services']:
			defansy.service_scan_semaphore = asyncio.Semaphore(config['max_scans'])
		else:
			defansy.port_scan_semaphore = asyncio.Semaphore(config['max_port_scans'])
			...
			if config['max_scans'] == config['max_port_scans']:
				defansy.service_scan_semaphore = defansy.port_scan_semaphore
			else:
				defansy.service_scan_semaphore = asyncio.Semaphore(config['max_scans'] - config['max_port_scans'])
	...
	[defansy.tags.append(t) for t in tags if t not in defansy.tags]
	...
	if args.exclude_tags != '':
		...
		# Remove duplicate lists from list.
		[defansy.excluded_tags.append(t) for t in excluded_tags if t not in defansy.excluded_tags]
	...
	for target in raw_targets:
		try:
			...
			found = False
			for t in defansy.pending_targets:
				if t.address == ip_str:
					found = True
					break
			...
			if isinstance(ip, ipaddress.IPv4Address):
				defansy.pending_targets.append(Target(ip_str, ip_str, 'IPv4', 'ip', defansy))
			elif isinstance(ip, ipaddress.IPv6Address):
				defansy.pending_targets.append(Target(ip_str, ip_str, 'IPv6', 'ip', defansy))
			else:
			...
			try:
				...
				try:
					...
					defansy.pending_targets.append(Target(target, ip, 'IPv4', 'hostname', defansy))
					...
					try:
						...
						defansy.pending_targets.append(Target(target, ip, 'IPv6', 'hostname', defansy))
					except socket.gaierror:
	...
	if not args.disable_sanity_checks and unresolvable_targets == True:
		error('defansy will not run if any targets are invalid / unresolvable. To override this, re-run with the --disable-sanity-checks option.')
		errors = True

	if len(defansy.pending_targets) == 0:
		error('You must specify at least one target to scan!')
		errors = True

	if config['single_target'] and len(defansy.pending_targets) != 1:
		error('You cannot provide more than one target when scanning in single-target mode.')
		errors = True

	if not args.disable_sanity_checks and len(defansy.pending_targets) > 256:
		error('A total of ' + str(len(defansy.pending_targets)) + ' targets would be scanned. If this is correct, re-run with the --disable-sanity-checks option to suppress this check.')
		errors = True

	if not config['force_services']:
		port_scan_plugin_count = 0
		for plugin in defansy.plugin_types['port']:
			if config['port_scans'] and plugin.slug in config['port_scans']:
				matching_tags = True
				excluded_tags = False
			else:
				matching_tags = False
				for tag_group in defansy.tags:
					if set(tag_group).issubset(set(plugin.tags)):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in defansy.excluded_tags:
					if set(tag_group).issubset(set(plugin.tags)):
						excluded_tags = True
						break
	...
	pending = []
	i = 0
	while defansy.pending_targets:
		pending.append(asyncio.create_task(scan_target(defansy.pending_targets.pop(0))))
		i+=1
	...
	while pending:
		...
		for task in done:
			if defansy.pending_targets:
				pending.add(asyncio.create_task(scan_target(defansy.pending_targets.pop(0))))
			if task in pending:
				pending.remove(task)

		port_scan_task_count = 0
		for targ in defansy.scanning_targets:
	...
	RESULTS_DIR = '/home/amine/amine/stage1/ss/defansy/results'
	...
	if len(defansy.completed_targets) > 1:
		for plugin in defansy.plugin_types['report']:
			if config['reports'] and plugin.slug in config['reports']:
				matching_tags = True
				excluded_tags = False
			else:
			...
			if matching_tags and not excluded_tags:
				pending.add(asyncio.create_task(generate_report(plugin, defansy.completed_targets)))
	...
	if timed_out:
		...
		elapsed_time = calculate_elapsed_time(start_time)
		warn('{byellow}defansy took longer than the specified timeout period (' + str(config['timeout']) + ' min). Cancelling all scans and exiting.{rst}')
	else:
	...
	if defansy.missing_services:
		warn('{byellow}defansy identified the following services, but could not match them to any plugins based on the service name. Please report these to Tib3rius: ' + ', '.join(defansy.missing_services) + '{rst}')
