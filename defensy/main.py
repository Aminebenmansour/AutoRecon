...
from defensy.config import config, configurable_keys, configurable_boolean_keys
from defensy.io import slugify, e, fformat, cprint, debug, info, warn, error, fail, CommandStreamReader
from defensy.plugins import Pattern, PortScan, ServiceScan, Report, defensy
from defensy.targets import Target, Service
...
if not os.path.exists(config['config_dir']):
	...
	if not os.path.exists(os.path.join(config['config_dir'], 'VERSION-' + VERSION)):
		warn('It looks like the config in ' + config['config_dir'] + ' is outdated. Please remove the ' + config['config_dir'] + ' directory and re-run defensy to rebuild it.')
...
if not os.path.exists(config['data_dir']):
	...
	if not os.path.exists(os.path.join(config['data_dir'], 'VERSION-' + VERSION)):
		warn('It looks like the plugins in ' + config['data_dir'] + ' are outdated. Please remove the ' + config['data_dir'] + ' directory and re-run defensy to rebuild them.')
...
def cancel_all_tasks(sig, frame):
	...
	for target in defensy.scanning_targets:
...
async def get_semaphore(defensy):
	semaphore = defensy.service_scan_semaphore
	while True:
		# If service scan semaphore is locked, see if we can use port scan semaphore.
		if semaphore.locked():
			if semaphore != defensy.port_scan_semaphore: # This will be true unless user sets max_scans == max_port_scans
				...
				if not defensy.pending_targets and (config['max_port_scans'] - port_scan_task_count) >= 1: # If no more targets, and we have room, use port scan semaphore.
					if defensy.port_scan_semaphore.locked():
						await asyncio.sleep(1)
						continue
					semaphore = defensy.port_scan_semaphore
					break
				else: # Do some math to see if we can use the port scan semaphore.
					if (config['max_port_scans'] - (port_scan_task_count + (len(defensy.pending_targets) * config['port_scan_plugin_count']))) >= 1:
						if defensy.port_scan_semaphore.locked():
							await asyncio.sleep(1)
							continue
						semaphore = defensy.port_scan_semaphore
...
async def port_scan(plugin, target):
	...
	async with target.defensy.port_scan_semaphore:
...
async def service_scan(plugin, service):
	semaphore = service.target.defensy.service_scan_semaphore

	if not config['force_services']:
		semaphore = await get_semaphore(service.target.defensy)
	...
	while plugin_pending:
		...
		if plugin.max_global_instances and plugin.max_global_instances > 0:
			async with service.target.defensy.lock:
				# Count currently running plugin instances.
				for target in service.target.defensy.scanning_targets:
		...
		async with semaphore:
			...
			nmap_extra = service.target.defensy.args.nmap
			if service.target.defensy.args.nmap_append:
				nmap_extra += ' ' + service.target.defensy.args.nmap_append
...
async def generate_report(plugin, targets):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(defensy)
...
async def scan_target(target):
	...
	if config['force_services']:
		...
		for plugin in target.defensy.plugin_types['port']:
			...
			if config['port_scans'] and plugin.slug in config['port_scans']:
				...
				matching_tags = False
				for tag_group in target.defensy.tags:
					if set(tag_group).issubset(plugin_tag_set):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in target.defensy.excluded_tags:
					if set(tag_group).issubset(plugin_tag_set):
						excluded_tags = True
						break
	...
	async with defensy.lock:
		defensy.scanning_targets.append(target)
	...
	while pending:
		...
		for service in services:
			...
			nmap_extra = target.defensy.args.nmap
			if target.defensy.args.nmap_append:
				nmap_extra += ' ' + target.defensy.args.nmap_append
			...
			for plugin in target.defensy.plugin_types['service']:
			...
			if not service_match:
				...
				if service.name not in config['service_exceptions'] and service.full_tag() not in target.defensy.missing_services:
					target.defensy.missing_services.append(service.full_tag())

	for plugin in target.defensy.plugin_types['report']:
	...
	async with defensy.lock:
		defensy.completed_targets.append(target)
		defensy.scanning_targets.remove(target)
...
async def run():
	...
	parser.add_argument('-c', '--config', action='store', type=str, default=config_file, dest='config_file', help='Location of defensy\'s config file. Default: %(default)s')
	parser.add_argument('-g', '--global-file', action='store', type=str, help='Location of defensy\'s global file. Default: %(default)s')
	...
	parser.add_argument('--timeout', action='store', type=int, help='Specifies the maximum amount of time in minutes that defensy should run for. Default: %(default)s')
	...
	parser.add_argument('--proxychains', action='store_true', help='Use if you are running defensy via proxychains. Default: %(default)s')
	...
	parser.add_argument('--accessible', action='store_true', help='Attempts to make defensy output more accessible to screenreaders. Default: %(default)s')
	parser.add_argument('-v', '--verbose', action='count', help='Enable verbose output. Repeat for more verbosity.')
	parser.add_argument('--version', action='store_true', help='Prints the defensy version and exits.')
	parser.error = lambda s: fail(s[0].upper() + s[1:])
	args, unknown = parser.parse_known_args()

	errors = False

	defensy.argparse = parser

	if args.version:
		print('defensy v' + VERSION)
		sys.exit(0)
	...
	if not args.config_file:
		unknown_help()
		fail('Error: Could not find config.toml in the current directory or ~/.config/defensy.')
	...
	if not config['plugins_dir']:
		unknown_help()
		fail('Error: Could not find plugins directory in the current directory or ~/.config/defensy.')
	...
	for plugins_dir in plugins_dirs:
		for plugin_file in sorted(os.listdir(plugins_dir)):
			if not plugin_file.startswith('_') and plugin_file.endswith('.py'):
				...
				try:
					spec = importlib.util.spec_from_file_location("defensy." + filename[:-3], os.path.join(dirname, filename))
					plugin = importlib.util.module_from_spec(spec)
					spec.loader.exec_module(plugin)

					clsmembers = inspect.getmembers(plugin, predicate=inspect.isclass)
					for (_, c) in clsmembers:
						if c.__module__ in ['defensy.plugins', 'defensy.targets']:
							continue
	...
	for plugin in defensy.plugins.values():
		if plugin.slug in defensy.taglist:
			unknown_help()
	...
	if len(defensy.plugin_types['port']) == 0:
		unknown_help()
		fail('Error: There are no valid PortScan plugins in the plugins directory "' + config['plugins_dir'] + '".')

	# Sort plugins by priority.
	defensy.plugin_types['port'].sort(key=lambda x: x.priority)
	defensy.plugin_types['service'].sort(key=lambda x: x.priority)
	defensy.plugin_types['report'].sort(key=lambda x: x.priority)

	if not config['global_file']:
		unknown_help()
		fail('Error: Could not find global.toml in the current directory or ~/.config/defensy.')
						...
						if 'pattern' in pattern:
							try:
								...
								if 'description' in pattern:
									defensy.patterns.append(Pattern(compiled, description=pattern['description']))
								else:
									defensy.patterns.append(Pattern(compiled))
							except re.error:
								unknown_help()
		...
		if key == 'global' and isinstance(val, dict): # Process global plugin options.
			for gkey, gval in config_toml['global'].items():
				if isinstance(gval, bool):
					for action in defensy.argparse._actions:
						if action.dest == 'global.' + slugify(gkey).replace('-', '_'):
							if action.const is True:
								action.__setattr__('default', gval)
							break
				else:
					if defensy.argparse.get_default('global.' + slugify(gkey).replace('-', '_')):
						defensy.argparse.set_defaults(**{'global.' + slugify(gkey).replace('-', '_'): gval})
		elif isinstance(val, dict): # Process potential plugin arguments.
			for pkey, pval in config_toml[key].items():
				if defensy.argparse.get_default(slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_')) is not None:
					for action in defensy.argparse._actions:
						if action.dest == slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'):
							if action.const and pval != action.const:
								...
								errors = True
							break
					defensy.argparse.set_defaults(**{slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'): pval})
		else: # Process potential other options.
			key = key.replace('-', '_')
			if key in configurable_keys:
				other_options.append(key)
				config[key] = val
				defensy.argparse.set_defaults(**{key: val})
	...
	if args.list:
		...
		if type in ['plugin', 'plugins', 'port', 'ports', 'portscan', 'portscans']:
			for p in defensy.plugin_types['port']:
	...
	if config['max_plugin_target_instances']:
		for plugin_instance in config['max_plugin_target_instances']:
			...
			if len(plugin_instance) == 2:
				if plugin_instance[0] not in defensy.plugins:
	...
	for slug, plugin in defensy.plugins.items():
	...
	if not errors:
		if config['force_services']:
			defensy.service_scan_semaphore = asyncio.Semaphore(config['max_scans'])
		else:
			defensy.port_scan_semaphore = asyncio.Semaphore(config['max_port_scans'])
			...
			if config['max_scans'] == config['max_port_scans']:
				defensy.service_scan_semaphore = defensy.port_scan_semaphore
			else:
				defensy.service_scan_semaphore = asyncio.Semaphore(config['max_scans'] - config['max_port_scans'])
	...
	[defensy.tags.append(t) for t in tags if t not in defensy.tags]
	...
	if args.exclude_tags != '':
		...
		# Remove duplicate lists from list.
		[defensy.excluded_tags.append(t) for t in excluded_tags if t not in defensy.excluded_tags]
	...
	for target in raw_targets:
		try:
			...
			found = False
			for t in defensy.pending_targets:
				if t.address == ip_str:
					found = True
					break
			...
			if isinstance(ip, ipaddress.IPv4Address):
				defensy.pending_targets.append(Target(ip_str, ip_str, 'IPv4', 'ip', defensy))
			elif isinstance(ip, ipaddress.IPv6Address):
				defensy.pending_targets.append(Target(ip_str, ip_str, 'IPv6', 'ip', defensy))
			else:
			...
			try:
				...
				try:
					...
					defensy.pending_targets.append(Target(target, ip, 'IPv4', 'hostname', defensy))
					...
					try:
						...
						defensy.pending_targets.append(Target(target, ip, 'IPv6', 'hostname', defensy))
					except socket.gaierror:
	...
	if not args.disable_sanity_checks and unresolvable_targets == True:
		error('defensy will not run if any targets are invalid / unresolvable. To override this, re-run with the --disable-sanity-checks option.')
		errors = True

	if len(defensy.pending_targets) == 0:
		error('You must specify at least one target to scan!')
		errors = True

	if config['single_target'] and len(defensy.pending_targets) != 1:
		error('You cannot provide more than one target when scanning in single-target mode.')
		errors = True

	if not args.disable_sanity_checks and len(defensy.pending_targets) > 256:
		error('A total of ' + str(len(defensy.pending_targets)) + ' targets would be scanned. If this is correct, re-run with the --disable-sanity-checks option to suppress this check.')
		errors = True

	if not config['force_services']:
		port_scan_plugin_count = 0
		for plugin in defensy.plugin_types['port']:
			if config['port_scans'] and plugin.slug in config['port_scans']:
				matching_tags = True
				excluded_tags = False
			else:
				matching_tags = False
				for tag_group in defensy.tags:
					if set(tag_group).issubset(set(plugin.tags)):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in defensy.excluded_tags:
					if set(tag_group).issubset(set(plugin.tags)):
						excluded_tags = True
						break
	...
	pending = []
	i = 0
	while defensy.pending_targets:
		pending.append(asyncio.create_task(scan_target(defensy.pending_targets.pop(0))))
		i+=1
	...
	while pending:
		...
		if m >= 2:
				break
		# If something failed in scan_target, defensy.errors will be true.
		if defensy.errors:
		...
		for task in done:
			if defensy.pending_targets:
				pending.add(asyncio.create_task(scan_target(defensy.pending_targets.pop(0))))
			if task in pending:
				pending.remove(task)

		port_scan_task_count = 0
		for targ in defensy.scanning_targets:
	...
	RESULTS_DIR = '/home/amine/amine/stage1/ss/defensy/results'
	...
	if len(defensy.completed_targets) > 1:
		for plugin in defensy.plugin_types['report']:
			if config['reports'] and plugin.slug in config['reports']:
				matching_tags = True
				excluded_tags = False
			else:
			...
			if matching_tags and not excluded_tags:
				pending.add(asyncio.create_task(generate_report(plugin, defensy.completed_targets)))
	...
	if timed_out:
		...
		elapsed_time = calculate_elapsed_time(start_time)
		warn('{byellow}defensy took longer than the specified timeout period (' + str(config['timeout']) + ' min). Cancelling all scans and exiting.{rst}')
	else:
	...
	if defensy.missing_services:
		warn('{byellow}defensy identified the following services, but could not match them to any plugins based on the service name. Please report these to Tib3rius: ' + ', '.join(defensy.missing_services) + '{rst}')
