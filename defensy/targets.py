...
from defensy.io import e, info, warn, error

class Target:

	def __init__(self, address, ip, ipversion, type, defensy):
		self.address = address
		self.ip = ip
		self.ipversion = ipversion
		self.type = type
		self.defensy = defensy
		self.basedir = ''
		self.reportdir = ''
		self.scandir = ''
	...
	def extract_service(self, line, regex=None):
		return self.defensy.extract_service(line, regex)

	async def extract_services(self, stream, regex=None):
		return await self.defensy.extract_services(stream, regex)
	...
	async def execute(self, cmd, blocking=True, outfile=None, errfile=None, future_outfile=None):
		...
		process, stdout, stderr = await target.defensy.execute(cmd, target, tag, patterns=plugin.patterns, outfile=outfile, errfile=errfile)
